...
MSA = readRDS("/oak/stanford/groups/dpwall/users/briannac/phyloWAS/data/diabetes_150/msa_DECIPHER.rds")
save_dir = "/oak/stanford/groups/dpwall/users/briannac/phyloWAS/results/diabetes_150/phyloWAS_DECIPHER"
...
### Perform phylo-was.
n_permutations = 100
phenotypes = sample_data(ps)$t1d == 'T1D' ## Change to whatever column has relevant phenotype!
bases = c('A','T','C','G')
n_cores = 15
idx = sample_data(ps)$host_subject_id # Change to whatever column has family info! (Or if no family info, then just do host.)
paired = F
