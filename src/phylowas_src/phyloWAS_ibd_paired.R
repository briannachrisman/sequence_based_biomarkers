...
ps = readRDS("/oak/stanford/groups/dpwall/microbiome/ncbi-datasets/jansson_twins_ibd/final/ps.rds")
MSA = readRDS("/oak/stanford/groups/dpwall/users/briannac/phyloWAS/data/jansson_twins_ibd/msa_DECIPHER.rds")
save_dir = "/oak/stanford/groups/dpwall/users/briannac/phyloWAS/results/jansson_twins_ibd/phyloWAS_DECIPHER"
...
### Perform phylo-was.
n_permutations = 100
phenotypes = sample_data(ps)$disease_stat == 'healthy'
bases = c('A','T','C','G')
n_cores = 15
idx = sample_data(ps)$pair_number ### Change to approrpriate column.
paired = T
